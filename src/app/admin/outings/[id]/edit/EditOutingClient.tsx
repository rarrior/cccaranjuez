...
import type { Member, Outing, Attendance } from "@/lib/types";
import { updateOuting } from "../../actions";

interface EditOutingClientProps {
  outing: Outing;
  seasonYear: number;
  members: Member[];
...
export default function EditOutingClient({ outing, seasonYear, members, attendances }: EditOutingClientProps) {
  const initialAttendees = new Set(attendances.map((a) => a.member_id));
  const initialCompleted = new Set(attendances.filter((a) => a.completed).map((a) => a.member_id));
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const result = await updateOuting(fd);
    if (result?.error) {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-fg">Editar Salida</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <input
                ...
                defaultValue={outing.description ?? ""}
